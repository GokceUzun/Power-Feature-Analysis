...
class Filter:
    ...
    def __init__(self, unfiltered_data, channels=[i for i in range(16)]): # by default, the 16 channels are selected
        self.unfiltered_data = unfiltered_data
        self.noise_index = (
            []
        )  # Index tracker to keep tract of the indices that are discarded
    ...
    def butter_bandpass(self):
        ...
        butter_b, butter_a = signal.butter(
            self.order, [self.low, self.high], btype="band", analog=False
        )
    ...
    def reshape_filtered_data(self, filtered_data):
        ...
        return reshaped_data[self.channels]
...
print(len(packet_loss_array))
print(len(packet_loss_idx))
"""
